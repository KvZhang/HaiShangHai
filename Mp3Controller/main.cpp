...
void stateWaiting() {
	if (myComm.flag.bit.isNeedProcess) {
		if (myComm.cmdHandler(&myCommRevContent)) {
			switch(myCommRevContent.CMD){
			case COMM_SINGLE_PLAY:
				myMp3.write(SINGLE_PLAY,myCommRevContent.data);
				break;
			case COMM_REPEAT_PLAY:
				myMp3.write(REPEAT_PLAY,myCommRevContent.data);
				break;
			case COMM_SET_VOLUME:
				myMp3.write(SET_VOLUMN,myCommRevContent.data);
				break;
			case COMM_STOP_PLAY:
			case COMM_RESET_ME:
				myMp3.write(PAUSE_PLAY,NONE_SONG);
